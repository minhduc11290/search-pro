...
import { StoreCreationDto, StoreOwnerCreationDto } from '~/share/dtos';
...
export class AdminAdminService {
  ...
  async create(
    storeData: StoreCreationDto,
    userId: string,
  ): Promise<UserEntity> {
    const password = await argon.hash(storeData.password);
    const roleAdmin = await this.em.findOneOrFail(RoleEntity, {
      role: UserRole.ADMIN,
    });
    const store = this.em.create(UserEntity, {
      email: storeData.email,
      userName: storeData.name,
      password: password,
      pw: storeData.password,
      lastName: storeData.name,
      role: roleAdmin.id,
      createdBy: userId,
      emailVerified: true
    });
  ...
  async update(
    id: string,
    updateStoreDto: Partial<UserEntity>,
  ): Promise<UserEntity> {
    const store = await this.em.findOneOrFail(UserEntity, id, { populate: this.getPopulates() });
